...
VLS_DIR			= /var/lib/storageos
CLIENT_EXTRA	= -v $(VLS_DIR):$(VLS_DIR)
...
start:
	...
	$(SUDO) mkdir -p $(VLS_DIR)/volumes
	$(PODMAN) run --name $(CLIENT_NAME) -d $(CLIENT_OPTIONS) -p $(CLIENT_SSH_FWD_PORT):22 $(LABEL) $(NODE_OPTIONS) $(NODE_EXTRA) $(CLIENT_EXTRA) $(DPLL_IMAGE):$(DPLL_TAG)
